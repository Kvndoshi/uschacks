...
SENSITIVE_CLICK_PATTERNS = re.compile(
    r"(click|press|tap|hit)\b.{0,30}\b(send|submit|post|publish|confirm|pay|purchase|delete|remove|order|place order|checkout)",
    re.IGNORECASE,
)

SENSITIVE_URL_PATTERNS = [
    r"checkout", r"payment", r"pay\b", r"delete",
...
def detect(agent_output: str, url: str = "", dom_context: str = "") -> tuple[bool, str]:
    ...
    for action in SENSITIVE_ACTIONS:
        if action.replace("_", " ") in output_lower or action in output_lower:
            return True, f"Sensitive action: {action}"

    if SENSITIVE_CLICK_PATTERNS.search(agent_output):
        match = SENSITIVE_CLICK_PATTERNS.search(agent_output)
        return True, f"Sensitive click detected: {match.group(0) if match else 'click action'}"
